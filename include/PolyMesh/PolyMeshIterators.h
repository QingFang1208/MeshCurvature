...
class BaseIterator {
 ...
 public:
  BaseIterator(const PolyMesh* mesh, const IH in, const OH out)
      : mesh_(mesh),
        cur_(in),
        ref_(out){
#ifdef DEBUG
        ...
        BaseIterator(const PolyMesh* mesh, const IH in)
      : mesh_(mesh),
        cur_(in),
        ref_(nullptr){
#ifdef DEBUG
        ...
        BaseIterator()
      : mesh_(nullptr), cur_(nullptr), ref_(nullptr) {
  }
  virtual ~BaseIterator() {}

  bool operator==(const BaseIterator& rhs) const {
    return (this->cur_ == rhs.cur_() && this->ref_ == rhs.ref_ &&
  ...
  const OH operator*() const { return ref_; }

  BaseIterator& operator=(const BaseIterator& rhs) {
    this->cur_ = rhs.cur_pointer();
    this->ref_ = rhs.ref_pointer();
    this->mesh_ = rhs.mesh();
    return *this;
  }

  const OH cur_pointer() const { return cur_; }
  const IH ref_pointer() const { return ref_; }
