...
/**
 * The RGB is 0-255, ����������С��0����
 * The RGBf is 0-1; С��0�����ᱻ����Ϊ0��
 */
...
struct MRGBA {
  ...
  MRGBA(unsigned int red, unsigned int green, unsigned int blue,
        unsigned int alpha) {
    r = red % 255;
    g = green % 255;
    b = blue % 255;
...
inline MRGBA RGBAF_to_RGBA(MRGBAf& color) {
  return (MRGBA(static_cast<unsigned int>(color.r * 255),
                static_cast<unsigned int>(color.g * 255),
                static_cast<unsigned int>(color.b * 255),
                static_cast<unsigned int>(color.a * 100)));
}
...
inline bool equality(Texcoord& lhs, Texcoord& rhs) {
  bool t0 =
      (lhs[0] - rhs[0] < ColorThreshold) && (rhs[0] - lhs[0] < ColorThreshold);
  bool t1 =
      (lhs[1] - rhs[1] < ColorThreshold) && (rhs[1] - lhs[1] < ColorThreshold);
  bool t2 =
      (lhs[2] - rhs[2] < ColorThreshold) && (rhs[2] - lhs[2] < ColorThreshold);
