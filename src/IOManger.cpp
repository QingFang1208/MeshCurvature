...
void remove_duplicated_vertices(std::vector<size_t>& _indices) {
  ...
  for (std::vector<size_t>::iterator iter = _indices.begin(); iter != endIter;
       ++iter)
...
bool OBJReader::can_u_read(const std::string& _filename) const {
  ...
  if (pos != std::string::npos)
    ...
    extension = _filename;  // check, if the whole filename defines the
                            // extension
...
bool OBJReader::read(const std::string& _filename, PolyMesh* mesh,
  ...
  if (!ifile.is_open() || !ifile.good()) {
    std::ostringstream error_info;
    error_info << "[OBJReader] : cannot not open file " << _filename
               << std::endl;
...
bool OBJReader::loadMeshFromOBJ(std::istream& _in, PolyMesh* mesh,
    ...
    if (keyWrd == "v") {
      ...
      if (!stream.fail()) {
        if (user_options.vert_have_texture) {
          ...
          stream >> w;
          if (!stream.fail()) texcoords[texcoords.size() - 1].setW(w);
        }
      } else {
        std::ostringstream error_info;
        error_info << "[OBJReader] : Only single 2D or 3D texture coordinate "
                      "per vertex"
                   << "allowed!" << std::endl;
        Massage::Error(error_info.str());
        return false;
  ...
  while (_in && !_in.eof()) {
    ...
    if (keyWrd == "mtllib") {
      ...
      if (matStream) {
        if (!read_material_obj(matStream)) {
          std::ostringstream error_info;
          error_info << "[OBJReader] : Could not read material file properly! "
                     << std::endl;
          Massage::Warning(error_info.str());
        }
        matStream.close();
        ...
        std::ostringstream error_info;
        error_info << "[OBJReader] : Material file '" << matFile
                   << "' not found!\n";
        Massage::Warning(error_info.str());
      }

      for (MaterialList::iterator material = materials_.begin();
           material != materials_.end(); ++material) {
        // Save the texture information in a property
        if ((*material).second.has_map_Kd())
          mesh->add_texture_information((*material).second.map_Kd_index(),
                                        (*material).second.map_Kd());
      }
    ...
    // negative indices
            ...
            case 0:  // vertex
              ...
              if (file_options.vert_have_color) {
                if ((size_t)(value - 1) < colors.size()) {
                  ...
                  std::ostringstream error_info;
                  error_info << "[OBJReader] : Error setting vertex color "
                             << std::endl;
                  Massage::Warning(error_info.str());
                }
              }
            ...
            case 1:  // texture coord
              ...
              if (file_options.vert_have_texture) {
                if (!texcoords.empty() &&
                  ...
                  std::ostringstream error_info;
                  error_info
                      << "[OBJReader] : Error setting Texture coordinates "
                      << std::endl;
                  Massage::Warning(error_info.str());
                }
              }
            ...
            case 2:  // normal
              ...
              if (file_options.vert_have_normal) {
                ...
                if ((unsigned int)(value - 1) < normal_list.size()) {
                  ...
                  std::ostringstream error_info;
                  error_info << "[OBJReader] : Error setting vertex normal "
                             << std::endl;
                  Massage::Warning(error_info.str());
                }
              }
      ...
      if (faceVertices.size() > 2) fh = mesh->addPolyFace(faceVertices);
  ...
  if (mesh->numPolygons() == 0) {
    ...
    if (colors.size() >= mesh->numVertices()) {
      if (user_options.vert_have_color && file_options.vert_have_color) {
        for (size_t i = 0; i < vertnum; i++) vert_list[i]->setColor(colors[i]);
      }
    }
  }
...
bool OBJReader::read_material_obj(std::fstream& _in) {
  ...
  while (_in && !_in.eof()) {
    ...
    if (_in.bad()) {
      std::ostringstream error_info;
      error_info << "[OBJReader Material] : Could not read file properly! "
                 << std::endl;
      Massage::Warning(error_info.str());
      return false;
    }
    ...
    else if (keyWrd == "Kd")  // diffuse color
    {
      ...
      if (!stream.fail()) mat.set_Kd(f1, f2, f3);
    }
    ...
    else if (keyWrd == "Ka")  // ambient color
    {
      ...
      if (!stream.fail()) mat.set_Ka(f1, f2, f3);
    }
    ...
    else if (keyWrd == "Ks")  // specular color
    {
      ...
      if (!stream.fail()) mat.set_Ks(f1, f2, f3);
    } else if (keyWrd == "map_Kd") {
      // Get the rest of the line, removing leading or trailing spaces
      // This will define the filename of the texture
      std::getline(stream, textureName);
      trimString(textureName);
      if (!textureName.empty()) mat.set_map_Kd(textureName, textureId++);
    } else if (keyWrd == "Tr")  // transparency value
    {
      stream >> f1;

      if (!stream.fail()) mat.set_Tr(f1);
    ...
    if (_in && indef && mat.is_valid() && !key.empty()) materials_[key] = mat;
  }
  return true;
}
...
bool OFFReader::read(const std::string& _filename, PolyMesh* mesh,
                     IOOptions& opt) {
  std::ifstream ifile(_filename.c_str(), std::ios::in);

  if (!ifile.is_open() || !ifile.good()) {
    std::ostringstream error_info;
    error_info << "[OFFReader] : cannot not open file " << _filename
               << std::endl;
    Massage::Error(error_info.str());
    return false;
  }
...
bool OFFReader::loadMeshFromOFF(std::istream& _in, PolyMesh* mesh,
                                IOOptions& opt) {
  if (!_in.good()) {
    std::ostringstream error_info;
  ...
  for (i = 0; i < nV && !_in.eof(); ++i) {
    ...
    if (options_.vert_have_normal) {
      ...
      if (user_options.vert_have_normal) pvert->setNormal(nx, ny, nz);
    }
    ...
    if (options_.vert_have_texture) {
      stream >> t0;
      stream >> t1;
      if (user_options.vert_have_texture) pvert->setTexture(t0, t1);
    }
  }
...
bool OFFReader::can_u_read(std::istream& _is) const {
  ...
  if (vertexDimensionTooHigh) return false;

  return true;
}

int OFFReader::getColorType(std::string& _line,
                            bool _texCoordsAvailable) const {
  /*
    0 : no Color
  ...
  */
  ...
  if (_line.size() < 1) return 0;

  // first remove spaces at start/end of the line
  while (_line.size() > 0 && isspace(_line[0])) _line = _line.substr(1);
...
bool OBJWriter::write(const std::string& _filename, PolyMesh* mesh,
                      IOOptions _opt, std::streamsize _precision) const {
  std::fstream out(_filename.c_str(), std::ios_base::out);

  if (!out) {
    std::ostringstream error_info;
    error_info << "[OBJWriter] : cannot not open file " << _filename
               << std::endl;
    Massage::Error(error_info.str());
    return false;
  }
...
bool OBJWriter::write(std::ostream& _out, PolyMesh* mesh, IOOptions _opt,
  ...
  if (_opt.face_has_color) {
    ...
    if (!matStream) {
      std::ostringstream error_info;
      error_info << "[OBJWriter] : cannot write material file " << matFile
                 << std::endl;
      Massage::Error(error_info.str());
      return false;
    } else {
  ...
  if (_opt.vert_have_texture || _opt.face_has_texcoord) {
    int texCount = 0;
    for (std::map<Texcoord, int>::iterator it = texMap.begin();
         it != texMap.end(); ++it) {
  ...
  bool onlyVertices = !_opt.vert_have_texture && !_opt.vert_have_normal &&
...
size_t OBJWriter::getMaterial(MRGBf& color) const {
  for (size_t i = 0; i < material_.size(); i++)
    if (equality(color, material_[i])) return i;
...
size_t OBJWriter::getMaterial(MRGBAf& color) const {
  for (size_t i = 0; i < materialA_.size(); i++)
    if (equality(color, materialA_[i])) return i;
...
bool OBJWriter::writeMaterial(std::ostream& _out, PolyMesh* mesh,
                              IOOptions _opt) const {
  MRGBf cf;
  MRGBAf caf;
  ...
  if (_opt.color_has_alpha)
    for (size_t i = 0; i < materialA_.size(); i++) {
      ...
      _out << "Kd " << materialA_[i].r << ' ' << materialA_[i].g << ' '
           << materialA_[i].b << '\n';
    ...
    for (size_t i = 0; i < material_.size(); i++) {
      _out << "newmtl " << "mat" << i << '\n';
      _out << "Ka 0.5000 0.5000 0.5000" << '\n';
      _out << "Kd " << material_[i].r << ' ' << material_[i].g << ' '
           << material_[i].b << '\n';
      _out << "illum 1" << '\n';
    }
...
bool OFFWriter::write(const std::string& _filename, PolyMesh* mesh,
                      IOOptions _opt, std::streamsize _precision) const {
...
bool OFFWriter::write(std::ostream& _os, PolyMesh* mesh, IOOptions _opt,
                      std::streamsize _precision) const {
  if (!_os.good()) {
    std::ostringstream error_info;
...
bool OFFWriter::write_ascii(std::ostream& _out, PolyMesh* mesh,
                            IOOptions _opt) const {
  size_t nV, nF;
  MPoint3 v;
  MVector3 n;
